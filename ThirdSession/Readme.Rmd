---
title: "Lab3 – Machine Learning Workshop R codes"
...
Now are trying to practice the machine learning for iris data. First we call   the packages:
...
```{r}
iris_df$Species = as.factor(ifelse(iris_df $ Species == "versicolor", "Yes", "No"))

# set the column names in the dataset
colnames(iris_df) <- c("Sepal.Length",
                        "Sepal.Width",
                        "Petal.Length",
                        "Petal.Width",
                        "Species")
...
Training_index <- createDataPartition(iris_df$Species,
                                     p = 0.7,
                                     list = FALSE)
# Training Data
    Training_data <- iris_df[Training_index, ]
        dim(Training_data)
          sapply(Training_data, class)
            head(Training_data)
                levels(Training_data$Species)

# select 30% of the data for Testing
Testing_data <- iris_df[-Training_index , ]



# summarize the class distribution
percentage <- prop.table(table(Training_data$Species)) * 100
          cbind(freq = table(Training_data$Species), percentage = percentage)
...
conmatrix_svm2 <- confusionMatrix(as.factor(predictions_svm2 ),
    ...
    outcome_clean <- factor(outcome, levels = c(0, 1), labels = c("No", "Yes"))

outcome_clean <- droplevels(outcome_clean)
      ##### create ROC
roc_svm_test2 = roc(response  = outcome_clean,
                     predictor = predictions_svm_test2$Yes,
                     plot       = TRUE,
                     ci         = TRUE,
                     legacy.axes= TRUE,
                     percent    = TRUE,
